...
function fee_create_using(param) {
    try {
        ...
        var sInsertSQL = 'INSERT INTO "zhimin.demo.db::demo.FEE" ("UID", "REF_VEHICLE.UID", "CREATED_ON", "CREATED_BY", "CHANGED_ON", "CHANGED_BY", "FEE_START_DATE", "FEE_END_DATE","FEE") ' 
                + ' values (?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
function fee_delete_using(param) {
    try {
        ...
        var sUpdateSQL = 'UPDATE "zhimin.demo.db::demo.FEE" SET "DELETE_FLAG" = \'1\', "CHANGED_ON" = ?, "CHANGED_BY" = ? ' 
                + ' WHERE "UID" = ?';
...
function fee_update_using(param) {
    try {
        ...
        var sUpdateSQL = 'UPDATE "zhimin.demo.db::demo.FEE" SET "CHANGED_ON" = ?, "CHANGED_BY" = ?, "FEE_START_DATE" = ?, "FEE_END_DATE" = ?, "FEE" = ? ' 
                + ' WHERE "UID" = ?';
        var updatePrepareStatement = param.connection.prepareStatement(sUpdateSQL);
...
service namespace "zhimin.demo.services"  { 
    ...
    create using "zhimin.demo.xsjslib:fee.xsjslib::fee_create_using"
    delete using "zhimin.demo.xsjslib:fee.xsjslib::fee_delete_using"
